     ...
     */
     ...
     function sendPushNotification($data){
        if(!empty($data['user_id'])){
            $userDevice = User::where('id', $data['user_id'])->select('id','device_token','device_type')->first();
          
            if(!empty($userDevice) && !empty($userDevice['device_token']) && $userDevice['device_token']!='string'){
                ...
                $gigsId = (!empty($data['gigs_id'])) ? $data['gigs_id'] : '';
                $url = 'https://fcm.googleapis.com/fcm/send';
                if($userDevice['device_type'] == 'ios') {
                    $fields = array('registration_ids' => $deviceId,'notification' => array('sound' => 'default','body' => $messages, 'data' => array('message' => $messages, 'type' => $data['type'],'title' => $data['title'],'gigs_id' => $gigsId,'type'=>$data['type'])));
                } else {
                    $fields = array('registration_ids' => $deviceId,'data' => array('message' => $messages, 'type' => $data['type'],'title' => $data['title'],'gigs_id' => $gigsId));
                   
                }
                Log::debug('notification check', ['fields' => $fields]);
                $headers = array(
